...
## ❓JPA에서 DAO와 DTO를 사용하는 이유는 무엇인가요?
- **DAO(Data Access Object)** : DB의 데이터 접근을 위한 객체입니다. JPA에서는 엔티티를 DB에 저장하거나 조회할 때 EntityManger를 직접 쓰지 않고 DAO(repository)를 따로 둬서, 비지니스 로직(Service)과 데이터 접근 로직을 분리합니다. 그럼 나중에 DB나 ORM 기술이 바뀌어도 Service 코드를 수정할 필요가 없어 유지보수가 쉬워집니다.
- **DTO(Data Transfer Object)** : 각 계층 간의 데이터를 교환하기 위한 객체입니다. 엔티티를 외부에 그대로 노출하면 내부 DB 구조가 노출될 수 있고, 엔티티는 변경 가능한 객체라 잘못된 값이 들어오면 DB에 영향을 줄 수도 있습니다. 그래서 DTO를 사용해 필요한 데이터만 전달하고 외부와 내부 구조를 분리합니다.

</br>

## ❓ORM 에 대해서 설명해보세요.
ORM은 자바 객체와 데이터베이스 테이블을 자동으로 매핑해주는 기술입니다. </br>
원래는 DB에 데이터를 저장하려면 SQL을 써야하지만, ORM을 사용하면 개발자가 엔티티 객체만 만들면 ORM이 알아서 SQL을 만들고 DB에 저장하거나 조회해줍니다.

**➕ JPA란 무엇인가요?** </br>
자바에서 ORM을 사용할 수 있도록 정의한 표준 API 입니다. 실제 구현체는 Hibernate이고, Spring Data JPA는 JPA를 더 편리하게 사용할 수 있도록 도와주는 기술입니다. 

**➕ 영속성 컨텍스트란 무엇인가요?** </br>
JPA가 엔티티를 잠시 저장해두고 관리하는 임시 메모리 공간입니다.
- **1차 캐시** : 같은 트랜잭션 안에서 동일한 ID로 조회하면 DB에 다시 접근하지 않고 1차 캐시에서 가져옴
- **동일성 보장** : 같은 트랜잭션 내에서는 같은 식별자의 엔티티는 항상 같은 객체
- **트랜잭션을 지원하는 쓰기 지연** : persist()를 호출해도 바로 DB에 INSERT 하지 않고, 트랜잭션 커밋 시점에 모아서 한번에 실행
- **변경 감지** : 영속 상태의 엔티티 값이 바뀌면 트랜잭션 커밋 시 자동으로 UPDATE 쿼리 실행
- **지연 로딩** : 연관된 엔티티는 실제로 사용하는 시점에 조회
