...
**➕ 객체지향이란 무엇인가?** </br>
객체지향은 프로그램을 기능 중심이 아니라 객체 중심으로 바라보는 설계 방식입니다. 필요한 객체를 먼저 정의하고, 각 객체의 역할과 책임을 나눠 객체들끼리 상호작용하면서 전체 기능을 구현하는 것입니다.
...
static class와 static method는 둘 다 객체를 만들지 않고 사용할 수 있다는 공통점이 있습니다. 하지만 목적은 조금 다릅니다. 
- static class는 바깥 클래스의 인스턴스와 상관없이 독립적으로 동작하는 중첩 클래스입니다. 주로 바깥 클래스와 개념적으로는 관련 있지만, 객체 상태에는 의존하지 않는 역할을 분리할 때 사용합니다.
- static method는 객체가 가지는 상태와 무관한 로직을 표현하기 위한 메서드로, 유틸리티 성격의 기능이나 순수 계산 로직에 주로 사용됩니다.

**➕ static 을 사용하면 어떤 이점을 얻을 수 있나요? 어떤 제약이 걸릴까요?** </br>
static을 사용하면 객체를 생성하지 않고 바로 사용할 수 있기 때문에 메모리 사용이 줄고 호출도 단순해집니다. 또한 이 메서드나 클래스는 “객체 상태에 의존하지 않는다”는 점을 코드로 명확하게 드러낼 수 있다는 장점이 있습니다. static 멤버는 클래스가 로딩될 때 한 번만 생성되기 때문에 전역적으로 공유되는 기능이나 값에 적합합니다.

반면 단점도 있습니다. static 메서드는 다형성이 적용되지 않아 오버라이딩이 불가능하고, 호출 대상이 컴파일 시점에 이미 결정됩니다. 그래서 확장성이 떨어지고 테스트나 의존성 주입이 필요한 환경에서는 사용하기 까다롭습니다. 잘못 사용하면 객체지향적인 설계보다는 절차적인 코드가 되기 쉽다는 점도 제약입니다.

**➕ 컴파일 과정에서 static 이 어떻게 처리되는지 설명해 주세요.** </br>
static은 컴파일 시점에 이 멤버는 객체가 아니라 클래스에 속한다는 것이 확정됩니다. 그래서 메서드 호출도 런타임에 결정되지 않고, 컴파일 시점에 이미 어떤 메서드를 호출할지가 정해집니다. 이후 JVM이 클래스를 로딩할 때 static 변수와 static 메서드는 Method Area에 한 번만 올라가고, 프로그램이 종료될 때까지 유지됩니다. 실행 시에는 객체를 만들지 않고 클래스 기준으로 바로 접근해 호출됩니다.
...
## ❓깊은 복사와 얕은 복사의 차이
**깊은 복사**는 새로운 객체를 만들어 객체 내부 값까지 모두 새로 복사하는 것입니다. 복사된 객체는 원본과 완전히 독립적이고 한쪽을 수정해도 다른 쪽에는 영향을 주지 않습니다. </br>
**얕은 복사**는 객체의 참조값(주소값)만 복사하는 것입니다. 두 변수가 같은 객체를 가리키고 한쪽에서 객체 내부 값을 변경하면 다른 쪽에도 영향을 줍니다. </br>
얕은 복사냐 깊은 복사냐의 차이는 참조 대상이 공유되는지, 아니면 새로운 객체가 생성되는지의 차이일 뿐이며 자바의 호출 방식은 항상 Call by Value 입니다. 
```java
Person p1 = new Person("민지");
Person p2 = p1; // 얕은 복사 → 참조값(주소)만 복사됨
p2.name = "지수";
System.out.println(p1.name); // 지수

Person p1 = new Person("민지");
Person p2 = new Person(p1.name); // 깊은 복사 → 새로운 객체 생성
p2.name = "지수";
System.out.println(p1.name); // 민지 (원본은 그대로)
```
