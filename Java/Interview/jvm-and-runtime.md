...
- **Java SE** (Standard Edition)는 우리가 흔히 말하는 자바의 기본 기능을 제공하는 표준 에디션입니다. 예를 들어, 컬렉션, 입출력, 스레드, 예외 처리 같은 핵심 라이브러리와 API가 포함되어 있습니다.
- **Java EE** (Enterprise Edition)는 SE를 기반으로 확장된 기업용 플랫폼입니다. Servlet, JSP, JPA, EJB 같은 웹/서버 개발에 필요한 기술과 API를 포함하고 대규모 트랜잭션 처리, 보안, 분산 처리 등 엔터프라이즈 환경에 적합한 기능을 제공합니다.

</br>

## ❓직렬화(Serialization)와 역직렬화(Deserialization)란 무엇인가?
자바 객체는 메모리(Heap) 안에 존재하기 때문에 그 상태로는 파일로 저장하거나 네트워크로 전송할 수 없습니다. 그래서 객체를 바이트 단위의 데이터로 변환하는데 이를 **직렬화**라고 합니다. 반대로 **역직렬화**는 바이트 스트림 형태의 데이터를 다시 자바 객체로 복원하는 과정입니다. 

**➕ SerialVersionUID를 선언해야 하는 이유** </br>
직렬화된 객체에는 클래스 정보와 SerialVersionUID라는 고유 식별자가 포함됩니다. </br>
만약 SerialVersionUID를 명시하지 않으면 JVM이 클래스 구조를 기반으로 자동 생성하여, 클래스의 필드나 구조가 조금이라도 변경되면 이 UID 값이 달라질 수도 있습니다. </br>
역직렬화 시 JVM은 현재 클래스의 UID와 직렬화된 데이터에 저장된 UID를 비교하여 일치하지 않으면 실패합니다. </br>
즉, UID를 명시하면 클래스 변경 시에도 버전 호환 여부를 개발자가 직접 통제할 수 있기 때문에 예기치 않은 역직렬화 실패를 방지할 수 있습니다.
